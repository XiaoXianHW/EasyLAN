...
public class GuiShareToLanEdit {
    ...
    public static class GuiShareToLanModified extends ShareToLanScreen {
        ...
        public void init() {
            ...
            PortTextBox.setResponder(text -> {
                PortText = text;
                refreshLanButtonState();
            });
            MaxPlayerBox.setResponder(text -> {
                MaxPlayerText = text;
                refreshLanButtonState();
            });

            this.addRenderableWidget(PortTextBox);
            this.addRenderableWidget(MaxPlayerBox);
            ...
            for (Widget widget : this.renderables) {
                if (widget == PortTextBox || widget == MaxPlayerBox) {
                    continue;
                }
